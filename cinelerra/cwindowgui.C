...
// Get target keyframe
			...
			if(!gui->affected_x && !gui->affected_y && !gui->affected_z)
			{
				...
				if(gui->translating_zoom)
				{
					gui->affected_z = 
						(FloatAuto*)gui->cwindow->calculate_affected_auto(
							affected_z_autos, 1, &created, 0);  // don't redraw
						
					// note: new floatauto nodes will be interpolated automatically
				...
				else
				{
					gui->affected_x = 
						(FloatAuto*)gui->cwindow->calculate_affected_auto(
							affected_x_autos, 1, &created, 0);  // don't redraw
			...
			if(gui->translating_zoom)
			{
				float new_z = gui->center_z + (cursor_y - gui->y_origin) / 128 ;
				if( new_z < 0 ) new_z = 0;
