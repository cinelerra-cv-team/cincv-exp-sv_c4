...
float FloatAutos::calculate_bezier(FloatAuto *prev, FloatAuto *next,int64_t position)
// Calculate interpolating Bézier Function for y-value
{
	float y0 = prev->value();
	float y3 = next->value();

// control points	
	float y1 = prev->value() + prev->control_out_value();
	float y2 = next->value() + next->control_in_value();
	float t = (float)(position - prev->position) / 
